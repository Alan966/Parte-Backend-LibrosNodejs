...
describe('DELETE /shortDb/delete/:id', () => {
    beforeAll(async () => {
        await mongoose.connect(mongodb.url)
    })

    afterAll(async () => {
        await mongoose.disconnect(mongodb.url)
    })

    let room;
    let response;
    beforeEach(async() => {
        room = await shortDb.create({
            listing_url:"https://cdn-images.farfetch-contents.com/15/45/41/10/15454110_28385859_1000.jpg",
            name:"queOvole",
            summary:"Algo Rico",
            space:"There is the most expensive",
        ...
        response = await request(app).delete(`/shortDb/delete/${room._id}`).send();
    })
    ...
    test('Borra correctamente', async () => {
        ...
        const foundRoom = await shortDb.findById(room._id)
