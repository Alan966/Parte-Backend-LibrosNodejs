     ...
     const data = req.body
         Header.findOneAndUpdate({_id: param}, data, (err, result) => {
            if(err){
                ...
                res.status(200).send('Update header with success')
            }
         })
} )
...
router.post('/delete/:id', (req, res) => {
    const param = req.params.id
    Header.findOneAndDelete({_id: param}, (err, result) => {
        if(err){
            res.status(500).send('Error with delete header')
        }else{
            console.log('Usuario eliminado con exito')
            res.status(200).send('Delete header with success')
        }
    })
})
